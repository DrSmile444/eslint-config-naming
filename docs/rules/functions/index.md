...
function doWork() { // WHY: camelCase - consistent and the final rule of the config enforces this
  return true;
}

// Exported function - PascalCase sometimes allowed for factories
export function CreateUser() { // WHY: PascalCase allowed for factory-style exports, but final camelCase rule may override in strict configs
  return {};
}
```
...
```ts
function Do_Work() { // WHY: Mixed PascalCase/underscores is inconsistent and disallowed by final rules
  return false;
}
```
